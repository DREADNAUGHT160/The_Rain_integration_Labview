...
# Added: IP/Port input fields + Reconnect button + live update of connection box.

import sys, json, socket, threading, time
from PyQt6.QtWidgets import (
    QApplication, QWidget, QPushButton, QVBoxLayout, QHBoxLayout,
    QLabel, QLineEdit, QGridLayout, QFrame, QCheckBox, QSizePolicy,
    QSpacerItem, QSpinBox, QStackedWidget, QFormLayout
)
...
class FogSystemGUI(QWidget):
    def __init__(self):
        ...
        self.reconnect_flag = False  # NEW flag for reconnect
        ...
        # ===== Left: IP/Port → ON/OFF (top) → modes → mode panel =====
        left = QVBoxLayout(); left.setSpacing(SP)

        # === NEW: IP/Port input ===
        ip_port_box = QHBoxLayout()
        self.ip_edit = QLineEdit(self.tcp_ip); self.ip_edit.setPlaceholderText("IP")
        self.port_edit = QLineEdit(str(self.tcp_port)); self.port_edit.setPlaceholderText("Port")
        self.port_edit.setFixedWidth(80)
        self.btn_reconnect = QPushButton("Reconnect")
        self.btn_reconnect.clicked.connect(self.request_reconnect)
        ip_port_box.addWidget(QLabel("IP:")); ip_port_box.addWidget(self.ip_edit)
        ip_port_box.addWidget(QLabel("Port:")); ip_port_box.addWidget(self.port_edit)
        ip_port_box.addWidget(self.btn_reconnect)
        left.addLayout(ip_port_box)
        ...
        # ON/OFF
        self.btn_onoff = bigbtn("ON/OFF")
        self.btn_onoff.toggled.connect(lambda v: self._set("b1", bool(v)))
        ...
        for i,b in enumerate((self.btn_manual, self.btn_interval, self.btn_vis)):
        ...
        self.cb_vs_start = QCheckBox("Start regulation")
        self.cb_vs_start.toggled.connect(lambda v: self._set("b18", bool(v)))
        ...
        QShortcut(QKeySequence("Ctrl+R"), self, self.request_reconnect)
    ...
    def request_reconnect(self):
        ...
        print(f"🔄 Reconnecting to {self.tcp_ip}:{self.tcp_port} ...")
    ...
    def _select_mode(self, which: str):
        if which=="manual":
            ...
            self.btn_interval.setChecked(True)
    ...
    def _set(self, key, val):
        self.state[key] = val; self._neutralize_for_mode()
    ...
    def tcp_loop(self):
        ...
        while True:
            try:
                if not self.connected or self.reconnect_flag:
                ...
                self.connected = False; self.c.update_connection.emit(False); time.sleep(0.3)
