...
class RainSystemGUI(QWidget):
    def __init__(self):
        ...
        self.resize(900, 560)            # starting size (resizable)
        self.setMinimumSize(700, 420)    # don't let it get too small
        ...
        # Framing mode for sending: "ndjson" or "len+nl"
        self.framing_mode = "ndjson"     # << change to "len+nl" if LabVIEW uses length header

        # Control state (Python -> LabVIEW)
        self.state = {"com":0,"v1":0,"v2":0,"v3":0,"v4":0,"v5":0,"v6":0,"cv":0,"wv":0,"int":0}

        SP = 10  # unified spacing
        ...
        left_col = QVBoxLayout(); left_col.setContentsMargins(0, 0, 0, 0); left_col.setSpacing(SP)
        ...
        def add_btn(text, key, r, c):
            ...
            b.setMinimumSize(120, 90)   # stays usable at small sizes
        ...
        # Top row labels changed per your screenshot
        self.btn_com = add_btn("ON/OFF", "com", 0, 0)
        self.btn_cv  = add_btn("Suplen", "cv",  0, 1)  # maps to cv
        self.btn_wv  = add_btn("Wasser", "wv",  0, 2)  # maps to wv
        ...
        bottom = QHBoxLayout(); bottom.setContentsMargins(0, 0, 0, 0); bottom.setSpacing(SP)
        ...
        right_col = QVBoxLayout(); right_col.setContentsMargins(0, 0, 0, 0); right_col.setSpacing(SP)
        ...
        # blue value bars (span both columns)
        self.labels = {}
    ...
    # ---------- SENDER with framing + diagnostics ----------
    def send_json(self):
        if not self.connected:
            return
        try:
            # intensity as integer (0 if empty)
            self.state["int"] = int(self.intensity_input.text()) if self.intensity_input.text() else 0

            # compact JSON string
            json_str = json.dumps(self.state, separators=(",", ":"))

            if self.framing_mode == "ndjson":
                # JSON + newline
                payload = (json_str + "\n").encode("utf-8")
            elif self.framing_mode == "len+nl":
                # ASCII length header + newline + raw JSON bytes
                body = json_str.encode("utf-8")
                header = f"{len(body)}\n".encode("ascii")
                payload = header + body
            else:
                raise ValueError(f"Unknown framing_mode: {self.framing_mode}")

            # diagnostics
            print("📤 Sending JSON object:")
            print(json.dumps(self.state, indent=2))
            print(f"• framing_mode = {self.framing_mode}")
            print(f"• JSON chars len = {len(json_str)}")
            print(f"• bytes len sent = {len(payload)}")
            print(f"• first 100 bytes = {repr(payload[:100])}")
            if self.framing_mode == "len+nl":
                header_preview = payload.split(b"\n", 1)[0]
                print(f"• header (ascii digits) = {header_preview!r}")
    ...
    def _extract_json_objects(self, buffer: str):
        """Return (list_of_complete_json_strings, leftover_buffer)."""
    ...
    def tcp_loop(self):
        ...
        while True:
            try:
                ...
                # Try to extract all complete JSON objects (newline not required)
